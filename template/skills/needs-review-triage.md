...
## Sequence
1. `syllabot status` lists the bucket: course, title, the text as written, the reason, and a candidate date when the engine had one.
...
3. For each answer:
   - **Owner gives a date**: write a partial Reading in `~/syllabot-state/readings/owner/<date>/<course>.json` with `"partial": true`, `"source_url": "owner:<today>"`, and one item whose `title` is the exact believed title and whose `date_text` is the owner's answer. Run `syllabot plan --readings <that folder>`, apply approved ops as in the nightly skill, `commit`, then `syllabot resolve-review "<key>" --note "owner: <answer>"` so it stops appearing while the page text is unchanged.
   - **Owner says skip**: `syllabot resolve-review "<key>" --note "owner: skip"`.
