...
> 4. Create two routines from `~/vidya/template/routines/nightly-2300.md` and `weekly-digest.md`. Use the quoted paragraph in each file as the routine instruction, with `<owner time zone>` replaced by `<TIMEZONE>`. Nightly syllabus check: every day at 11:00 PM. Weekly digest: every Sunday at 6:00 PM. Do not run them yet. Do not create the safety check-in routine now; it is created only if I turn check-ins on during setup.
...
- Decide on the optional safety check-in when the playbook offers it. If yes,
  connect Gmail with send permission and give the Bot the names and addresses of
  people who have agreed to be contacts; then create the routine in
  `routines/safety-checkin.md`.
