...
The part I'm proudest of is the shape, not the bot. LLM at the edges, code in the middle: one supervisor Bot, one cheap reader subagent per page, and a standard-library Python engine with 80 tests deciding what changed, what's ambiguous, what may be written, who may be messaged. The model applies exactly what the code approved. `selftest` prints ALL PASS in a second on a clean machine.
